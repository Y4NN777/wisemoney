...
routing, adapter, normalization, and aggregate-validation components. Module
...
C4Component
  ...
  Container_Boundary(pwa, "PWA Client") {
    ...
    Component(ai_orch_client, "AI Orchestration Client", "TypeScript", "The only module that knows provider details. In managed mode: attaches JWT and sends aggregate-only context to the Go edge. In BYO-key mode: decrypts a key in memory, applies client-side routing and fallback, and calls the provider directly (TB-04).")
...
C4Component
  ...
  Container_Boundary(go_edge, "Go Managed Edge") {
    ...
    Component(payload_cap, "Structural Payload Cap", "Go, strict JSON validation", "Applied to every managed AI request before dispatch. Validates the exact aggregate-only schema and rejects full-only or unknown fields with HTTP 400. Client headers cannot elevate the ceiling.")

    Component(provider_adapters, "Provider-Adapter Layer", "Go", "Managed adapters: OpenRouter Free, Gemini 3.6 Flash, and optional DeepSeek V4 Flash. Each translates the internal request format and normalizes the response. Unconfigured providers are omitted from routing. Endpoint URLs are hardcoded (M-PROXY-01).")
  ...
  SystemDb_Ext(postgres, "Postgres", "Users and hashed refresh-token records only.")
  System_Ext(ai_providers, "Managed AI Providers", "OpenRouter / Gemini / optional DeepSeek.")
...
| Go edge — in-flight | Redacted aggregates only | Full-only fields are rejected at the boundary |
| Go edge — persisted | Never | INV-PROXY-01: edge retains nothing financial after a request cycle |
| Postgres | Never | No financial schema; users and hashed refresh tokens only |
