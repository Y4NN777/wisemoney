      ...
      <main>
        <section className="border-b border-black/15">
          <div className="mx-auto grid max-w-[1440px] lg:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
            ...
            <div className="grid min-h-64 grid-cols-2 border-t border-black/15 lg:border-t-0">
              <div className="flex flex-col justify-between border-r border-black/15 bg-[#0077b6] p-5 text-white sm:p-7">
              ...
              <div className="help-orbit relative overflow-hidden bg-[#f7f7f8] p-5 sm:p-7" aria-hidden="true">
                ...
                <div className="absolute left-1/2 top-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#0077b6] bg-white" />
                ...
                <div className="absolute bottom-[20%] right-[16%] h-3 w-3 border border-[#0077b6] bg-white" />
              </div>
            </div>
          </div>
        </section>

        <section className="border-b border-black/15 bg-[#0077b6] text-white sm:hidden">
          <div className="px-4 py-5">
            ...
            <Button
              type="button"
              variant="secondary"
              className="mt-4 min-h-12 w-full justify-between rounded-none border border-white bg-white text-[#0077b6]"
          ...
          <div className="mx-auto grid max-w-[1440px] gap-0 lg:grid-cols-[minmax(250px,0.45fr)_minmax(0,1.55fr)]">
            ...
            <div className="relative bg-white p-2 sm:p-3">
              ...
              <Input
                ...
                className="h-12 border-black/20 bg-white pl-10 pr-14 text-base shadow-none hover:border-[#0077b6]/50 focus-visible:border-[#0077b6] focus-visible:ring-[#0077b6]"
              />
        ...
        <div className="mx-auto grid max-w-[1440px] lg:grid-cols-[minmax(250px,0.45fr)_minmax(0,1.55fr)]">
          <nav aria-label={t("helpPage.contents")} className="hidden border-r border-black/15 p-8 lg:block">
            ...
            <ol className="space-y-1">
              {sections.map((section, index) => (
                <li key={section.id}>
                  <a href={`#${section.id}`} className="grid grid-cols-[2.25rem_1fr] py-2 text-sm leading-tight text-black/65 hover:text-[#0077b6]">
          ...
          <section aria-label={t("helpPage.results")} className="min-w-0">
            {query.length > 0 && (
              <div className="border-b border-black/15 bg-[#e6f5fa] px-4 py-3 text-sm sm:px-8">
                    ...
                    <li id={section.id} key={`${section.id}-${query}`} className="scroll-mt-20">
                      <details className="group" open={query.length > 0 || window.location.hash === `#${section.id}`}>
                        <summary className="grid cursor-pointer list-none grid-cols-[4.25rem_1fr_2.5rem] gap-3 px-4 py-7 transition-colors hover:bg-[#f7f7f8] sm:grid-cols-[6.5rem_1fr_3rem] sm:px-8 sm:py-9">
                          ...
                          <span className="flex h-8 w-8 items-center justify-center border border-black/20 text-xl leading-none text-[#0077b6] group-open:bg-[#0077b6] group-open:text-white">
                          ...
                          <ol className="divide-y divide-black/10">
                            ...
                            {section.id === "installation" && (
                              <li className="px-4 py-5 sm:px-8">
                                <div className="border-l-2 border-[#0077b6] bg-white p-4">
        ...
        <section className="border-y border-black/15 bg-[#0077b6] text-white">
