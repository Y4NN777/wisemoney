...
const fr: ProductTask[] = [
  ...
  },
  {
    ...
    summary: "Ajoutez un revenu ou une dépense puis retrouvez-le dans Toutes les opérations.",
    prerequisites: ["Avoir au moins un compte actif."],
    steps: ["Ouvrez Saisie puis Transaction.", "Choisissez Revenu ou Dépense, le compte, la catégorie, le montant et la date.", "Choisissez Enregistrer.", "Pour corriger ou supprimer l’opération, ouvrez Toutes les opérations depuis le tableau de bord puis sélectionnez sa ligne."],
  ...
  },
  {
    ...
    prerequisites: ["Avoir ouvert l’espace privé."],
    steps: ["Choisissez la période à analyser en haut du tableau de bord.", "Lisez le solde total comme l’argent disponible maintenant.", "Comparez revenus, dépenses et différence pour la période.", "Ouvrez Toutes les opérations pour rechercher et filtrer chaque mouvement."],
...
const englishCopy: Record<string, TranslatedTaskCopy> = {
  ...
  "transactions": { title: "Record, correct, or delete a transaction", summary: "Add income or an expense and find it again in All operations.", prerequisites: ["Have at least one active account."], steps: ["Open Capture, then Transaction.", "Choose Income or Expense, the account, category, amount, and date.", "Choose Save.", "To correct or delete it, open All operations from the dashboard and select its row."], expectedResult: "The account balance and period indicators are recalculated.", aliases: ["income", "expense", "movement", "edit operation", "history"], limitations: ["Deleting a transaction is permanent in the current cycle."] },
  "virements": { title: "Transfer between accounts and track it", summary: "Move an amount between WiseMoney accounts or record money sent to an external destination.", prerequisites: ["Have an active source account; two active same-currency accounts for an internal transfer."], steps: ["Open Capture, then Transfer.", "Choose the source and destination accounts.", "Enter the amount, date, and optional note, then choose Record transfer.", "Open Dashboard, then All operations, and filter by Transfers to find it."], expectedResult: "The source is debited and the internal destination is credited.", aliases: ["transfer", "account to account", "send money", "external destination", "track transfer"], limitations: ["Internal accounts must share a currency; a transfer cannot be edited after recording."] },
  "tableau-de-bord": { title: "Read the dashboard and activity", summary: "Compare available balance, income, expenses, and the difference for a selected period.", prerequisites: ["Open the private space."], steps: ["Choose the period at the top of the dashboard.", "Read total balance as money available now.", "Compare income, expenses, and difference for the period.", "Open All operations to search and filter every movement."], expectedResult: "Indicators and charts follow the selected period while total balance stays current.", aliases: ["dashboard", "activity", "charts", "total balance", "all operations"], limitations: ["Accounts without a usable exchange rate may be excluded from the base-currency total."] },
