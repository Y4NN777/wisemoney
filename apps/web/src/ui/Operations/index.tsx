...
import { ArrowDownLeft, ArrowRightLeft, ArrowUpRight, CalendarDays, ListFilter, PiggyBank, Repeat2, Search, X } from "lucide-react";
import { useTranslation } from "react-i18next";
import { filterFinancialOperations, groupOperationsByLocalDay, operationCashTotals } from "../../analytics/operations.ts";
...
function operationTone(kind: FinancialOperationKind): string {
  if (kind === "income") return "border-positive/30 bg-positive-wash text-positive";
  if (kind === "expense" || kind === "planned_expense") return "border-negative/30 bg-negative-wash text-negative";
...
function operationContext(operation: FinancialOperation, snapshot: NonNullable<ReturnType<typeof useFinancialState>["data"]>, t: ReturnType<typeof useTranslation>["t"]): string {
  const values: string[] = [t(`operations.kinds.${operation.kind}`)];
...
function OperationAmount({ operation }: { operation: FinancialOperation }) {
  const amount = operation.displayAmount ?? operation.amount;
  if (amount == null) return null;
  const isIncome = operation.kind === "income" || (operation.kind === "recurring_realisation" && operation.direction === "income");
  const isExpense = operation.kind === "expense" || operation.kind === "planned_expense" || (operation.kind === "recurring_realisation" && operation.direction === "expense");
  const signed = isIncome ? "+" : isExpense ? "−" : "";
  return (
    <span className={`col-start-2 min-w-0 break-words text-sm font-semibold tabular-nums sm:col-start-auto sm:shrink-0 ${isIncome ? "text-positive" : isExpense ? "text-negative" : "text-foreground"}`}>
...
export default function Operations() {
  ...
  const now = Date.now();
  const start = search.start ?? ALL_TIME_START;
  ...
  const totals = useMemo(() => operationCashTotals(filtered, currency), [currency, filtered]);
  ...
  return (
    <main aria-label={t("operations.title")} className="app-page">
      ...
      <section aria-label={t("operations.summary")} className="grid divide-y divide-border border border-border bg-card sm:grid-cols-3 sm:divide-x sm:divide-y-0">
        {(["income", "expenses", "net"] as const).map((key) => (
          <div key={key} className="p-4">
            <p className="text-xs font-medium text-muted-foreground">{t(`operations.${key}`)}</p>
            <p className={`mt-1 text-xl font-semibold tabular-nums ${key === "income" && totals[key] > 0 ? "text-positive" : key === "expenses" && totals[key] > 0 ? "text-negative" : "text-foreground"}`}>
              {formatMoney({ minorUnits: totals[key], currency })}
            </p>
          </div>
        ))}
      </section>
      ...
      <Card>
        <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><ListFilter className="h-4 w-4" />{t("operations.filters")}</CardTitle></CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Select value={search.accountId ?? "all"} onValueChange={(value) => updateSearch({ accountId: value === "all" ? undefined : value })}>
            ...
            <SelectContent><SelectItem value="all">{t("operations.allAccounts")}</SelectItem>{snapshot.accounts.map((item) => <SelectItem key={item.id} value={item.id}>{item.name}</SelectItem>)}</SelectContent>
          </Select>
          ...
          <div className="grid grid-cols-2 gap-2 sm:col-span-2 xl:col-span-1">
            <Input aria-label={t("operations.startDate")} type="date" value={search.start == null ? "" : localDateInput(search.start)} onChange={(event) => updateSearch({ start: inputTimestamp(event.target.value, false) })} />
            <Input aria-label={t("operations.endDate")} type="date" value={search.end == null ? "" : localDateInput(search.end)} onChange={(event) => updateSearch({ end: inputTimestamp(event.target.value, true) })} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {operationsQuery.isLoading ? (
            ...
            const groupTotals = operationCashTotals(completeGroup.operations, currency);
            return (
            <section key={group.day} aria-labelledby={`operations-day-${group.day}`}>
              <h2 id={`operations-day-${group.day}`} className="flex flex-wrap items-center justify-between gap-2 border-y border-border bg-accent/45 px-4 py-2 text-xs font-semibold text-muted-foreground first:border-t-0">
                ...
                <span className="tabular-nums">{t("operations.daySubtotal", { amount: formatMoney({ minorUnits: groupTotals.net, currency }) })}</span>
              </h2>
                ...
                return (
                  <li key={operation.id} className="border-b border-border last:border-b-0">
                    <button type="button" onClick={() => setSelected(operation)} className="interactive-surface grid w-full grid-cols-[2.5rem_minmax(0,1fr)] items-center gap-3 px-4 py-3 text-left sm:grid-cols-[2.5rem_minmax(0,1fr)_auto]">
                      <span className={`flex h-10 w-10 items-center justify-center border ${operationTone(operation.kind)}`}><Icon className="h-4 w-4" /></span>
        ...
        <SheetContent side="right" className="w-full max-w-md sm:max-w-md">
          {selected != null && <>
            ...
            <dl className="mt-6 divide-y divide-border border-y border-border">
              ...
              <div className="grid gap-1 py-3 sm:grid-cols-[8rem_minmax(0,1fr)] sm:gap-3"><dt className="text-xs text-muted-foreground">{t("operations.amount")}</dt><dd className="min-w-0"><OperationAmount operation={selected} /></dd></div>
